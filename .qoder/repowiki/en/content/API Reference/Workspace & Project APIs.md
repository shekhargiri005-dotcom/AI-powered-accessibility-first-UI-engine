...
## Update Summary
**Changes Made**
- Removed all references to the `/api/engine-config` endpoint and associated functionality
- Updated architecture diagrams to reflect the removal of the centralized engine configuration system
- Revised security and encryption sections to focus on workspace-scoped settings only
- Updated troubleshooting guide to remove engine configuration-related issues
- Modified data models to reflect the current workspace settings structure
...
**Updated** Removed references to the centralized engine configuration system that has been deprecated in favor of workspace-scoped settings.
...
- Chunk Generation API: Generates code chunks using configured AI providers with workspace-scoped settings.
...
**Updated** Removed references to the centralized engine configuration system. The chunk generation API now uses workspace-scoped settings directly.
...
- [route.ts:36-153](file://app/api/chunk/route.ts#L36-L153)
...
- Purpose: Generate code chunks using configured AI providers with workspace-scoped settings.
- Authentication: Requires a valid session.
- Endpoints:
  - POST /api/chunk
    - Description: Generates a code chunk based on intent, manifest, and target file using the configured provider.
    - Request body: { intent: string, manifest: object, targetFile: string, model: string, maxTokens?: number, isMultiSlide?: boolean, provider?: string }.
    - Behavior: Validates parameters, gets workspace context from session and headers, and generates code using the configured provider.
    - Response: { success: true, code: string, safetyWarnings?: string[] }.
    - Status codes: 200 on success; 400 invalid parameters; 403 AI provider not configured; 500 internal error.

**Updated** Removed references to the engine configuration endpoint. The chunk generation API now uses workspace-scoped settings directly.

```mermaid
sequenceDiagram
participant Client as "Client"
participant Chunk as "Chunk Route"
participant Settings as "workspaceKeyService"
participant Adapter as "AI Adapter"
Client->>Chunk : "POST /api/chunk"
Chunk->>Settings : "getWorkspaceApiKey(provider, workspaceId)"
Settings-->>Chunk : "decrypted API key"
Chunk->>Adapter : "generateFileChunk(...)"
Adapter-->>Chunk : "generated code"
Chunk-->>Client : "{ success : true, code }"
```
...
- [route.ts:1-12](file://app/api/projects/[id]/route.ts#L1-L12)
- [route.ts:1-23](file://app/api/projects/[id]/rollback/route.ts#L1-L23)
- [route.ts:1-154](file://app/api/chunk/route.ts#L1-L154)
- [projectStore.ts:1-291](file://lib/projects/projectStore.ts#L1-L291)
- [auth.ts:1-87](file://lib/auth.ts#L1-L87)
- [workspaceKeyService.ts:1-138](file://lib/security/workspaceKeyService.ts#L1-L138)
- [encryption.ts:1-95](file://lib/security/encryption.ts#L1-L95)
...
**Updated** Removed troubleshooting entries related to the engine configuration endpoint.
...
**Updated** Removed references to the centralized engine configuration system that has been deprecated.
...
- POST /api/chunk
  - Request: Authorization: Bearer <JWT>; Body: { intent, manifest, targetFile, model, provider }
  - Response: 200 { success: true, code: "..." }

**Updated** Removed examples for the engine configuration endpoint.
