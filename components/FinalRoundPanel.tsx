...
function buildVisionCriteria(
  ...
  if (status === 'skipped') {
    return [
      { label: 'Vision model available', passed: false, detail: errorMessage ?? 'No multimodal model configured.' },
      { label: 'Visual review executed', passed: false, detail: 'Skipped before criteria evaluation.' },
      { label: 'Auto-repair decision', passed: false, detail: 'No pass/fail decision produced.' },
    ];
  }

  if (status === 'error') {
    return [
      { label: 'Vision model available', passed: true },
      { label: 'Visual review executed', passed: false, detail: errorMessage ?? 'Request failed.' },
      { label: 'Auto-repair decision', passed: false, detail: 'No pass/fail decision produced.' },
    ];
  }

  const score = result?.score ?? 0;
  const repairSuggested = !!result?.suggestedCode;
  return [
    { label: 'Quality score >= 82', passed: score >= 82, detail: `Score: ${score}/100` },
    { label: 'Critical visual/a11y issues absent', passed: !!result?.passed || !repairSuggested },
    { label: 'No repair required', passed: !repairSuggested, detail: repairSuggested ? 'Repair generated by Vision AI.' : 'Component approved as-is.' },
  ];
}
...
export default function FinalRoundPanel({
  ...
  const criteria = buildVisionCriteria(status, result, errorMessage);
    ...
    >
      ...
      >
        <div className="px-4 pb-4 space-y-3 border-t border-gray-800/60">
          ...
          {/* Explicit pass/fail verdict + criteria */}
          <div className="rounded-xl border border-gray-700/40 bg-gray-800/30 px-3 py-2.5 space-y-2">
            ...
            <p className="text-[11px] text-gray-400">
              Criteria passed: <span className="font-semibold text-gray-200">{passedCount}/{criteria.length}</span>
            </p>
            <div className="space-y-1.5">
              {criteria.map((item) => (
                <div key={item.label} className="flex items-start gap-2">
                  <span className={`mt-0.5 w-1.5 h-1.5 rounded-full ${item.passed ? 'bg-emerald-400' : 'bg-red-400'}`} />
                  <div>
                    <p className={`text-[11px] ${item.passed ? 'text-emerald-300' : 'text-red-300'}`}>{item.label}</p>
